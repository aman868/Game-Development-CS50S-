...
function Level:update(dt)
    ...
    --add more players if the space key was pressed, the player has not been launched, the player has not collided with anythging and there are not more than 3 player in game
    if love.keyboard.keysPressed['space'] and self.launchMarker.launched and not self.launchMarker.collided and #self.launchMarker.aliens < 3 then
        --x and y positions of the original alien
        ...
        --Make it apper above the original alien
        alienAbove = Alien(self.world, 'round', xPos, yPos - 45, 'Player')

        -- set the velocity to that of the first alien plus some extra on the x axis to make it go further
        alienAbove.body:setLinearVelocity(xVel + 30, yVel)

        -- make the alien pretty bouncy
        alienAbove.fixture:setRestitution(0.4)
        --Add friction to the rotation
        alienAbove.body:setAngularDamping(1)

        --Add the alien aliens to the table
        table.insert(self.launchMarker.aliens, alienAbove)

        --Make it apper below the original alien
        alienBelow = Alien(self.world, 'round', xPos, yPos + 45, 'Player')

        -- set the velocity to that of the first alien plus some extra on the x axis to make it go further
        alienBelow.body:setLinearVelocity(xVel - 30, yVel)

        -- make the alien pretty bouncy
        alienBelow.fixture:setRestitution(0.4)
        --Add friction to the rotation
        alienBelow.body:setAngularDamping(1)

        --Add the alien aliens to the table
        table.insert(self.launchMarker.aliens, alienBelow)
    ...
    if self.launchMarker.launched then
        for k, alien in pairs(self.launchMarker.aliens) do
            ...
            if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 2.5) then
                alien.body:destroy()
                self.launchMarker = AlienLaunchMarker(self.world)
