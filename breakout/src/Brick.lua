...
function Brick:init(x, y)
    ...
    self.width = 32
    self.height = 16
    --Used to determine if this brick contains a powerup
    self.powerUp = false
    --Used to determine if this brick is a lock one
    self.lock = false
    --Used to determine if this brick contains a key
    self.key = false
...
function Brick:hit()
    ...
    if self.lock and not keyObtained then
        return
    end
    ...
    -- if you are a lock brick then this will not affect you
    if self.tier > 0 then
...
function Brick:render()
    if self.inPlay then
        if self.lock then
            love.graphics.draw(gTextures['main'], gFrames['lock'],
                self.x, self.y)
        else
