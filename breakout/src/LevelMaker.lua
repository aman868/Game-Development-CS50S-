...
--Amount of power ups we are going to generate


function LevelMaker.createMap(level)
    local bricks = {}
    local powerUp = math.random(2,3)   

    local lockBlockChance = math.random(1, 10)    
    ...
    --If we have not excedded the amount of power ups we can generate per level we are going add a new one to a random brick
    for i = powerUp, 0, -1 do
        bricks[math.random(#bricks)].powerUp = true  
    end    

    --same for the lock brick
    if lockBlockChance >= 2 then
        lockIndex = math.random(#bricks)
        brick = bricks[lockIndex]
        if not brick.powerUp then
            brick.lock = true     
            brick.color = 1
            brick.tier = 0
            --If we added the lock brick we MUST add a key power up
            while true do 
                keyIndex = math.random(#bricks)
                if not bricks[keyIndex].lock and not bricks[keyIndex].powerUp then
                    bricks[keyIndex].key = true
                    break
                end
            end
        end
